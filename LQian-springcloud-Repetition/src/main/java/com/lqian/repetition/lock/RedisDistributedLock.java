...
public class RedisDistributedLock extends AbstractDistributedLock {
    ...
    private boolean setRedis(final String key, String value,final long expire) {
        try {
            //TODO 妈的 气死 不知都为何本工程lambda表达式编译不通过
    ...
    public boolean releaseLock(final String key) {
        ...
        try {
          ...
          /* RedisCallback<Long> callback = connection -> {
            ...
            logger.error("releaseLock redis occured an exception", e);
        }finally {
            //清除掉ThreadLocal 中的数据
            lockFlag.remove();
        }
